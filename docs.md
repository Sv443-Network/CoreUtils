...
- ⚠️ The data is cloned using [`structuredClone()`](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Structured_clone_algorithm) and serialized with [`JSON.stringify()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify). So make sure to only use data that can be cloned / serialized properly. Circular structures and complex objects (containing functions, symbols, etc.) will either not be cloned properly, throw an error on load and save, or cause otherwise unexpected behavior. Properties with a value of `undefined` will be removed from the data prior to saving it, so use `null` instead if you need to preserve the key.
